...
var Util = require('../../../tests/functional/localizedData/lib/util');
...
const getLocalizedTestData = testData => {
  ...
  if (Object.keys(allLocales).length <= 0) {
    ...
    for (let locale in allLocales) {
      ...
      let localeData = allLocales[locale];
      ...
      tests.sender = setupUser(tests.sender, localeData.sender, locale);
      tests.receiver = setupUser(tests.receiver, localeData.receiver, tests.locale);

            // if (localeData.receiver.hasOwnProperty("locales")) {
            //     for (let temp_locale in localeData.receiver.locales) {
            //         let tempTests = {};
            //         Object.assign(tempTests, tests);
            //         tempTests.receiver = setupUser(tests.receiver, localeData.receiver.locales[temp_locale], temp_locale);
            //         testsArray[(localeData.locale || locale) + temp_locale + ""] = tempTests;
            //     }
            // }else{
            //     testsArray[localeData.locale || locale] = tests
            // }
...
const getTestsData = (testCaseName, dataProviderFile) => {
  let testData = JSON.parse(
        readFile('tests/functional/spec/dataprovider/' + dataProviderFile)
    );
