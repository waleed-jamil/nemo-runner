...
let profile = function profile(cb) {
    ...
    */
    ...
    if (process.argv.slice(2).indexOf('-G') >= 0) {
        ...
        if (isDirectory(testcaseName)) {
            console.log('Running all tests in directory and subdirectories in: ' + testcaseName);
            allTestCases = allData.getAllTestsNames(testcaseName);
        } else if (isFile(testcaseName)) {
            console.log('Runing all tests in File: ' + testcaseName);
            allTestCases = allData.getTestsFromFile(testcaseName)

        } else {
            allTestCases = allData.getTestByName(testPath + 'spec/flow/', testcaseName)
    ...
    for (let testSuiteName in allTestCases) {
        allTestCases[testSuiteName].testcases.forEach(function (testCaseName) {
            ...
            let dataFileName = testCaseName.split('_')[1] + '.json';
            // check and set local data from the commandline args
            ...
            if (locales.length > 0) {
                allBases[testCaseName].data = data.getTestsDataByCountry(testCaseName, dataProviderFile, locales);
            } else {
                allBases[testCaseName].data = data.getTestsData(testCaseName, dataProviderFile);
            }
