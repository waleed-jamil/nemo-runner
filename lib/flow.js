...
let profile = function profile(cb) {
  ...
  if (process.argv.slice(2).indexOf('-M') >= 0) {
    ...
    dataProviderFile = 'p2pnodeweb/' + testcaseName.split('_')[1] + '.json' || 'p2pnodeweb/template.json';
  }


  if (locales.length > 0) {
    base.data = data.getTestsDataByCountry(testcaseName, dataProviderFile, locales);
  } else {
    base.data = data.getTestsData(testcaseName, dataProviderFile);
  }
